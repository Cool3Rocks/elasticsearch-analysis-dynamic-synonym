    ...
    public boolean isNeedReloadSynonymMap() {
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) { // 返回200 才做操作
                if (!response.getLastHeader(LAST_MODIFIED_HEADER).getValue().equalsIgnoreCase(lastModified)
                        || !response.getLastHeader(ETAG_HEADER).getValue().equalsIgnoreCase(eTags)) {
